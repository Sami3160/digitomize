            ...
            <div className="flex flex-row pt-24 px-10 pb-4 mt-10">
                ...
                <div className="w-10/12">
                    ...
                    <div className="bg-[#1a1a1a] rounded-xl shadow-lg p-6 w-full text-white overflow-auto scrollbar-hide">
                        <h2 className="text-2xl font-bold text-yellow-500 mb-3">Leetcode</h2>

                        {/* Profile Section with Badges Inline */}
                        <div className="flex gap-4 items-center mb-6">
                            <img src={leetcodedata?.profile?.avatar} alt="profileImage" className="w-16 h-16 rounded-full border-2 border-yellow-500 shadow-md" />
                            <div className="flex-1">
                                <p className="text-xl font-semibold">{leetcodedata?.profile?.username}</p>
                            ...
                            <div className="flex gap-3">
                                {leetcodedata?.badges?.badges?.slice(0, 4).map((badge, index) => (
                                    <img key={index} src={badge.icon} alt={badge.name} className="w-12 h-12" title={badge.displayName} />
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="flex gap-8 mt-4">
                            ...
                            <div className="bg-[#222222] rounded-lg py-6 px-4 w-1/2 text-xs text-gray-300 shadow-md">
                                <p className="font-semibold text-lg text-yellow-500 mb-4">Contest Stats</p>
                    ...
                    <div className="bg-[#1a1a1a] rounded-xl shadow-lg p-6 w-full overflow-auto text-white scrollbar-hide">
                        ...
                        {/* Profile Section */}
                        <div className="flex gap-4 items-center mb-6">
                            <img src={codeforcesdata?.profile?.avatar} alt="profileImage" className="w-16 h-16 rounded-full shadow-md border-2 border-blue-500" />
                            <div>
                                <p className="text-xl font-semibold">{codeforcesdata?.profile?.handle}</p>
                        ...
                        <div className="bg-[#222222] rounded-lg py-4 px-3 text-sm text-gray-300 shadow-md">
                            <p className="font-semibold text-lg text-blue-500 mb-4">Contest Stats</p>
                            <div className="flex flex-col space-y-4 text-md">
                                <div className="flex justify-between items-center font-bold">
                        ...
                        <div className="my-4">
                            <p className="text-xl font-semibold text-gray-300">Total Solved: <span className="text-blue-400">{codeforcesdata?.solved?.totalSolved}</span></p>
                            ...
                            <div className="flex flex-wrap gap-3 text-sm">
                                {Object.entries(codeforcesdata?.solved?.problemTags || {}).map(([tag, count], index) => (
                                    <span key={index} className="bg-[#1e293b] text-blue-400 px-3 py-2 rounded-full shadow-sm">
                                        {tag} x{count}
                                    </span>
                                ))}
                            </div>
                    ...
                    <div className="bg-[#1a1a1a] rounded-xl shadow-lg p-6 w-full overflow-auto text-white">
                        <h2 className="text-2xl font-bold text-green-500 mb-3">GeeksforGeeks</h2>

                        {/* Profile Section */}
                        <div className="flex gap-4 items-center mb-6">
                            <img src={gfgdata?.profile?.avatar} alt="profileImage" className="w-16 h-16 rounded-full shadow-md border-2 border-green-500" />
                            <div>
                                <p className="text-xl font-semibold">{gfgdata?.profile?.username}</p>
                                <p className="text-gray-400 text-sm">Rank: {gfgdata?.profile?.ranking}</p>
                            </div>
                        </div>

                        {/* Stats Section */}
                        <div className="bg-[#222222] rounded-lg py-4 px-3 text-sm text-gray-300 shadow-md">
                            <p className="font-semibold text-lg text-green-500 mb-4">Stats</p>
                            <div className="flex flex-col space-y-4 text-md">
                                <div className="flex justify-between items-center font-bold">
                                    <p>Total Problems Solved</p>
                                    <span className="text-green-500">{gfgdata?.stats?.totalSolved}</span>
                                </div>
                                <div className="flex justify-between items-center font-bold">
                                    <p>Level of Expertise</p>
                                    <span className="text-green-500">{gfgdata?.stats?.expertiseLevel}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Tags Section */}
                        <div className="my-4">
                            <p className="text-xl font-semibold text-gray-300">Popular Tags:</p>
                            <div className="flex flex-wrap gap-3 text-sm">
                                {gfgdata?.tags?.map((tag, index) => (
                                    <span key={index} className="bg-[#1e293b] text-green-400 px-3 py-2 rounded-full shadow-sm">
                                        {tag}
                                    </span>
                                ))}
                        </div>
